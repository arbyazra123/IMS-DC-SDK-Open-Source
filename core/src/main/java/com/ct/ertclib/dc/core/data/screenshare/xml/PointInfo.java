...
public class PointInfo implements XmlInfo {
    ...
    public void setEncodedData(String encodedData) {
        ...
        List<PointBean> decode = SketchEncoder.decode(mEncoding, encodedData.getBytes());
        if (decode.size() > 0) {
            mPointBean = decode.get(0);
        }
    }
    ...
    @Override
    public String toXMl() {
        if (TextUtils.isEmpty(mEncodedData) && mPointBean != null) {
            mEncodedData = new String(SketchEncoder.encode(mEncoding, mPointBean));
        }
        return "<point encoding=\"" + mEncoding + "\">" +
                mEncodedData +
                "</point>";
    }
}
